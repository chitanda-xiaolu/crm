 ...
 * @LastEditTime: 2020-04-27 17:08:15
...
<script>
import { editpConfigProduct, pconfigDetail } from '@/api/pconfig'
...
export default {
    ...
    methods: {
        ...
        showModal() {
            ...
            this.setDisable(1, filterData, 3)
            this.getDzTreeData()
            this.treeData = filterData
            this.getDetail()
        },
        getDetail() {
            pconfigDetail({ id: this.id }).then(result => {
                if(result.code === 200 && result.success) {
                    let res = result.data
                    res.parentId = typeof res.parentId === 'number' ? res.parentId.toString() : res.parentId
        ...
        dataFormSubmit() {
            this.$refs['dataForm'].validate(valid => {
                if (valid) {
                    ...
                    editpConfigProduct(params).then(result => {
                        if (result.code === 200 && result.success) {
