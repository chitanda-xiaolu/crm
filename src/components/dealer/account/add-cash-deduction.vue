...
export default {
    ...
    mounted() {},
    methods: {
        ...mapActions({ cashRdeductionData: 'cashRdeductionData', accountData: 'accountData' }),
        init(id) {
            this.visible = true
        ...
        validateMoney(rule, value, callback) {
            if(value <= this.accountMsg.accountInfo.cashTotal) {
                callback()
            }else {
                callback(new Error(`请输入正确金额,当前余额是：${this.accountMsg.accountInfo.cashTotal}`))
            }
        },
        // 金额自动匹配小数点
        adddot() {
